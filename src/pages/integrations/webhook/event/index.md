...
| Wait for Complete Batch | `bool` | False | All| If enabled, mParticle will POST events to you in batches only after a user session has ended, so that each event batch you receive will represent a full session of user activity within your app. This setting is no longer supported since the release of the mParticle iOS SDK version 8.0. Enable `Send as Batch` if you are using version 8.0 or later of the mParticle iOS SDK. |

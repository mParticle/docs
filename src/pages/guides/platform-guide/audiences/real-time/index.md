...
* The tier setting for events. If set to any tier except [**Personalize**](/guides/platform-guide/tiered-events), an event can't be used as criteria in a real-time audience and won't be evaluated. After being set to any other tier, the event is grayed out in real-time audience selection drop-downs.
...
<aside>If you have <a href="https://docs.mparticle.com/guides/platform-guide/tiered-events/">Tiered Events</a> functionality enabled, you won't be able to select an event in real-time audiences or calculated attributes if the tier is other than <b>Personalize</b>. To select such an event for real-time audiences or calculated attributes, it must be in the <b>Personalize</b> tier.</aside>
